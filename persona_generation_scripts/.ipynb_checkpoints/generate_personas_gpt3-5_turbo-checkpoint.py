...
output_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'new_data/gpt-3.5-turbo')
            ...
            for prompt_num, pr in enumerate(prompts):
                ...
                if response is not None:
                    for i in range(num_gens):
                        tempd = {'text': [response.choices[i].message.content], 'prompt_num': [prompt_num], 'model': ["gpt-3.5-turbo-0125"], 'gender': [gen], 'race': [r], 'prompt': [prompt]}
